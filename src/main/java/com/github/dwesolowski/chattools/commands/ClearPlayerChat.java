...
public class ClearPlayerChat implements CommandExecutor {
    ...
    public ClearPlayerChat(ChatTools plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String string, String[] args) {
        ...
        if ((cmd.getName().equalsIgnoreCase("clearplayerchat")) && (args.length == 0)) {
            if (player.hasPermission("chattools.clearplayerchat")) {
                for (int i = 0; i < 100; i++) {
                    sender.sendMessage("");
                }
                sender.sendMessage(ChatColor.translateAlternateColorCodes('&', this.plugin.getConfig().getString("Messages.ClearPlayerChat")));
                return true;
            } else {
